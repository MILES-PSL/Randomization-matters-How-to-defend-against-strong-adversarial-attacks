    ...
    def __init__(self, dataset, batch_size=1, shuffle=False, sampler=None,
        ...
        if sampler is None:  # give default samplers
            if self._dataset_kind == _DatasetKind.Iterable:
                ...
                sampler_shuffle = RandomSampler(dataset)
                sampler_sequential = SequentialSampler(dataset)

        if batch_size is not None and batch_sampler is None:
            # auto_collation without custom batch_sampler
            batch_sampler_shuffle = BatchSampler(sampler_shuffle, batch_size, drop_last)
            batch_sampler_sequential = BatchSampler(sampler_sequential, batch_size, drop_last)

        self.batch_size = batch_size
        self.drop_last = drop_last
        self.sampler_shuffle = sampler_shuffle
        self.sampler_sequential = sampler_sequential
        self.batch_sampler_shuffle = batch_sampler_shuffle
        self.batch_sampler_sequential = batch_sampler_sequential
